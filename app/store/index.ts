...
export const useStore = create<{
  ...
  generate: (prompt: string, presetName: PresetName) => void
  ...
  */
  generate: (prompt: string, presetName: PresetName) => {
  ...
  convertClapToComic: async (clap: ClapProject): Promise<{
    ...
    const cameraShots = clap.segments.filter((s: { category: any }) => s.category === ClapSegmentCategory.CAMERA)

    const shots = cameraShots.map((cameraShot: any) => ({
      camera: cameraShot,
